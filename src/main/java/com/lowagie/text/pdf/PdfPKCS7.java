...
public class PdfPKCS7 {
            ...
            for (Enumeration aliases = keystore.aliases(); aliases.hasMoreElements();) {
                try {
                    ...
                    if (ocsp.isSignatureValid(
                            new JcaContentVerifierProviderBuilder().setProvider(provider).build(certStoreX509))) {
                        return true;
                    }
                }
                catch (Exception ex) {
        ...
        try {
            for (Enumeration aliases = keystore.aliases(); aliases.hasMoreElements();) {
                try {
                    ...
                    SignerInformationVerifier verifier = new JcaSimpleSignerInfoVerifierBuilder().setProvider(provider).build(certStoreX509);
    ...
    public boolean isRevocationValid() {
        ...
        try {
            ...
            CertificateID tis = new CertificateID(dcp.get(CertificateID.HASH_SHA1),
